...
$\eta:\operatorname*{QSym}\rightarrow\operatorname*{QSym}$ be the $\mathbf{k}%
...
fact, in order to construct such a constrictor, we write our set $E$ as the
...
k, & \text{if }e=a;\\
k+1, & \text{if }e=b;\\
\beta\left(  e\right)  , & \text{if }e\in E_{2}%
\end{cases}
...
\otimes
\underbrace{ \left(
 \sum_{\varphi\text{ is a packed }{\mathbf{E}}\mid_{Q}\text{-partition}%
...
\operatorname{Adm}{\mathbf{E}}$ having the property that $\pi\mid_{P}$ is a
$\left(  P,>_{1},<_{2}\right)  $-partition and $\pi\mid_{Q}$ is a $\left(
Q,<_{1},<_{2}\right)  $-partition (by the definition of $Z$). Write $\alpha$
in this form.
\par
From $\left(  P,Q\right)  \in\operatorname{Adm}{\mathbf{E}}$, we see that
...
the orbit $O$ contains at least one element), we therefore only
