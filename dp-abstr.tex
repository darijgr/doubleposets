...
$e\parallel_{1}f$ if and only if $e$ and $f$ are not $<_{1}$-comparable. We
shall abbreviate the $k$-tuple $\left(  1,1,\ldots,1\right)  $ as $\left(
1^{k}\right)  $ for each $k\in\mathbb{N}$. Then, it is not hard to see that%
\begin{align*}
...
\eqref{pf.thm.antipode.Gammaw.signrev}. Hence,
