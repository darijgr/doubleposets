...
\begin{proof}
[Proof of Lemma \ref{lem.Gammaw.empty}.] Part (a) is obvious (since there is
only one $\mathbf{E}$-partition when $E=\varnothing$). Part (b) follows by
observing that $\Gamma\left(  \mathbf{E},w\right)  $ is a homogeneous power
series of degree $\sum_{e\in E}w\left(  e\right)  $ (and this degree is $>0$
when $E\neq\varnothing$).
\end{proof}
