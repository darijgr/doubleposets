...
$P \cap Q = \varnothing$ and $P \cup Q = E$).
Also, $Q \cup P = P \cup Q = E$. Furthermore,
the relation $<_1$ is the opposite relation of $>_1$ (since $>_1$
is the opposite relation of $<_1$). Also,
$\left(E, >_1, <_2\right)$ is a double poset, and the map
...
\ref{cor.antipode.Gamma} nevertheless holds? Computations using the SageMath
computer algebra system \cite{SageMath} have shown that, among the double
posets $\mathbf{E}=\left(  E,<_{1},<_{2}\right)  $ with $\left\vert
E\right\vert \leq6$ and with $<_{1}$ being a total order, \textbf{only} the
tertispecial ones satisfy the conclusion of Corollary \ref{cor.antipode.Gamma}%
. However, computer experimentation stops being viable shortly after the
$\left\vert E\right\vert =6$ threshold, and double posets for which $<_{1}$ is
not a total order have not been tested at all. Neither a positive nor a
...
5.25]{Gri-nbc}, which is uncannily reminiscent the involution $T$ in the proof
