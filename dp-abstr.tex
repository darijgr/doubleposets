...
\item[(b)] If $<$ is a strict partial order on a set $P$,
...
\item[(c)] If $<$ is a strict partial order on a set $P$,
and if $a$ and $b$ are two elements of $P$, then we say that
...
graph by $E$, and then define two strict partial orders $<_1$ and
$<_2$ on $E$ by
\[
...
and if not,
then at least our proofs below are more in the nature of things.
...
E\right)  =\left\{  1,2,\ldots,k\right\}  $ for some $k\in\mathbb{N}$.
...
and that $\tau$ is a packed ${\mathbf{E}}\mid_{Q}$-partition.
...
(since $\sigma$ is an ${\mathbf{E}}\mid_{P}$-partition). But the definition of
...
${\mathbf{E}}\mid_{Q}$-partition\footnote{\textit{Proof.} Assume that we are
...
f\right)  $ (since $\tau$ is an ${\mathbf{E}}\mid_{Q}$-partition). But the
...
We have $f \notin P$ and thus $f \in E \setminus P = Q$ (since
$P \cap Q = \varnothing$ and $P \cup Q = E$).
Also, $Q \cup P = P \cup Q = E$. Furthermore, $>_1$ is the
...
We shall use the shorthand word ``$g$-orbit'' for an orbit under the
action of $g$ on $E$. Thus, $u$ is a $g$-orbit. All $g$-orbits are
finite (since $E$ is a finite set). In particular, the $g$-orbit $u$ is
finite.

Since $u<_{1}^{g}u$, there exist $a\in u$ and $b\in u$ with
$a<_{1}b$. Consider these $a$ and $b$. There exists a $k\in\mathbb{Z}$ such
that $b=g^{k}a$ (since $a$ and $b$ both lie in one and the same $g$-orbit
$u$). Consider this $k$.
\par
The $g$-orbit $u$ of $a$ is finite.
Thus, there exists a positive integer $n$
such that $g^{n}a=a$. Consider this $n$. Notice that $g^{np}a=\left(
g^{n}\right)  ^{p}a=a$ for every $p\in\mathbb{Z}$ (since $g^{n}a=a$).
\par
...
strict partial order. Thus, $\EE^g
...
\begin{array}
[c]{c}%
\text{here, we have extended the sum to all }G\text{-orbits on }\operatorname{Par}{\mathbf{E}}
\\
...
\ref{thm.antipode.Gammaw} and \ref{thm.antipode.Gammaw}), we have obtained
formulas for a large class of quasisymmetric generating functions for maps
from a double poset to $\left\{  1,2,3,\ldots\right\}  $. Several natural
questions arise:

\begin{question}
What happens to the above results when $\mathbf{E}$ is no longer required to
be tertispecial? We know that the tertispeciality condition on $\mathbf{E}$ is
sufficient (say) for Corollary \ref{cor.antipode.Gamma}, but is it also
necessary, or are there non-tertispecial double posets for which Corollary
\ref{cor.antipode.Gamma} nevertheless holds? It is not hard to see
that, among the double
posets $\mathbf{E}=\left(  E,<_{1},<_{2}\right)  $ with $<_{1}$ being a
total order, \textbf{only} the tertispecial ones satisfy the conclusion of
Corollary \ref{cor.antipode.Gamma}.
% Why? It is clearly enough to consider the case when
% $E = \left\{1,2,\ldots,n\right\}$ and $<_1 = <_{\ZZ}$. In this case,
% $\Gamma\left(\EE\right) = M_{\left(1^n\right)}
% + \sum_k M_{\left(1^{k-1},2,1^{n-k}\right)}
% + \text{(a sum of } M_\alpha \text{ with } \ell(\alpha) < n-1
%    \text{)}$, where the first sum
% ranges over all $k$ that do not satisfy $k+1 <_2 k$. Apply the
% antipode $S$ using the formula in Example~\ref{exam.antipode.Gammaw}
% (b). Again, throw away all $M_\alpha$ with $\ell(\alpha) < n-1$.
% Compare with $\Gamma\left(\left(E, >_1, <_2\right)\right)$.
However, the question remains unanswered for the many
double posets for which $<_{1}$ is
not a total order. Neither a positive nor a
negative answer would surprise me.
\end{question}
...
\bibitem[Grin16]{Gri-nbc}Darij Grinberg,
