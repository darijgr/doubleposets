...
which a group acts on the double poset. The proofs are sketched
here, and will be detailed in a forthcoming paper.
...
literature\footnote{although the identity of the authors of
\cite{Mal-Reu} with those of \cite{Mal-Reu-DP} suggests that these
versions have already been found}, and if not,
...
(which will be used without mention):
...
\item[(b)]
If $S$ is a set and $\pi : S \to \left\{1, 2, 3, \ldots\right\}$ is
a map, then $\pi$ is said to be \textit{packed} if $\pi\left(S\right)$
is an initial interval. Clearly, this initial interval must be
$\left\{1, 2, \ldots, \left|\pi\left(S\right)\right|\right\}$.
...
w\left(e\right)$ for every $1 \leq i \leq \ell$, so that
...
\prod_{k = 1}^\ell \underbrace{x_{i_k}^{\alpha_k}}
                              _{\substack{= x_{i_k}^{\sum_{e \in \varphi^{-1}\left(k\right)}
                                w\left(e\right)} \\
                                = \prod_{e \in \varphi^{-1}\left(k\right)}
                                x_{i_k}^{w\left(e\right)}}}
\nonumber \\
&= \sum_{i_1 < i_2 < \cdots < i_\ell}
\underbrace{\prod_{k = 1}^\ell \prod_{e \in \varphi^{-1}\left(k\right)}
x_{i_k}^{w\left(e\right)}}
_{ = \prod_{e \in E} x_{i_{\varphi\left(e\right)}}^{w\left(e\right)}}
= \sum_{i_1 < i_2 < \cdots < i_\ell}
\prod_{e \in E} x_{i_{\varphi\left(e\right)}}^{w\left(e\right)}
\nonumber \\
&= \sum_{S \subseteq \left\{1, 2, 3, \ldots\right\} ; \ \left|S\right| = \ell}
\underbrace{\prod_{e \in E} x_{r_S\left(\varphi\left(e\right)\right)}^{w\left(e\right)}}
           _{=\xx_{r_S\circ\varphi,w}}
= \sum_{S \subseteq \left\{1, 2, 3, \ldots\right\} ; \ \left|S\right| = \ell}
\xx_{r_S\circ\varphi,w}
...
$S \subseteq \left\{1, 2, 3, \ldots\right\}$
such that $\left|S\right| = \ell$. The bijection sends a sequence
$\left(i_1 < i_2 < \cdots < i_\ell\right)$ to the set of its entries;
its inverse map sends every $S$ to the sequence
$\left(r_S\left(1\right), r_S\left(2\right), \ldots,
r_S\left(\left|S\right|\right)\right)$.}.
...
&= \sum_{S \subseteq \left\{1, 2, 3, \ldots\right\} ; \ \left|S\right| = \ell}
\sum_{\pi : E \to \left\{1, 2, 3, \ldots\right\} \text{; } \pack \pi = \varphi
\text{; } \pi\left(E\right) = S}
\xx_{\pi, w}
\end{align*}
(because if $\pi : E \to \left\{1, 2, 3, \ldots\right\}$ is a map
satisfying $\pack \pi = \varphi$, then
$\left|\pi\left(E\right)\right| = \ell$). But for every
$\ell$-element subset $S$ of
$\left\{1, 2, 3, \ldots\right\}$, there exists exactly
one $\pi : E \to \left\{1, 2, 3, \ldots\right\}$ satisfying
$\pack \pi = \varphi$ and $\pi\left(E\right) = S$ (namely,
$\pi = r_S \circ \varphi$), and therefore we have
\[
\sum_{\pi : E \to \left\{1, 2, 3, \ldots\right\} \text{; } \pack \pi = \varphi
\text{; } \pi\left(E\right) = S}
\xx_{\pi, w}
...
\text{; } \pi\left(E\right) = S}
\xx_{\pi, w}}_{= \xx_{r_S\circ\varphi,w}} \\
&= \sum_{S \subseteq \left\{1, 2, 3, \ldots\right\} ; \ \left|S\right| = \ell}
\xx_{r_S\circ\varphi,w}
= M_{\ev_w \varphi}
\end{align*}
...
\item[(b)] For
any subset $S$ of $E$, we let $\EE\mid_S$ denote the double poset
$\left(S, <_1, <_2\right)$, where $<_1$ and $<_2$ (by abuse of
...
is a double poset defined as in \cite[\S 2.1]{Mal-Reu-DP}, and we
refer to the forthcoming paper for the details of this simple fact.
...
\par
Assume first that $O$ is $E$-coeven. Thus, $\pi$ is even (by the definition of what
...
Thus, assume that $O$ is not $E$-coeven. Hence, $\pi$ is not
$E$-coeven.
