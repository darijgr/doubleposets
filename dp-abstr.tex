...
an earlier result by Gessel). We then further generalize this
...
sort of converse. Namely, let us assume that $\mathbf{k}=\mathbb{Z}$. If
...
$\eta: \QSym \rightarrow \QSym $ be the $\mathbf{k}%
...
$\mathbf{k}\left[  \left[  \mathbf{x}\right]  \right]  $ by a coefficientwise
...
\operatorname{Adm} \EE $, a packed $ \EE \mid_{P}$-partition
...
of $\mathcal{T}$, this means that $\left(  P,Q\right)  \in\operatorname{Adm}%
 \EE $, that $\sigma$ is a packed $ \EE \mid_{P}$-partition,
and that $\tau$ is a packed $ \EE \mid_{Q}$-partition.
...
\end{align}
Hence, $\varphi\mid_{Q}=\operatorname*{add}\nolimits_{k}\circ\tau$, so that
...
P,Q\right)  \in\operatorname{Adm} \EE $, that $\sigma$ is a packed
$ \EE \mid_{P}$-partition, and that $\tau$ is a packed ${\mathbf{E}%
}\mid_{Q}$-partition.
...
\begin{align*}
&  \sum_{\left(  P,Q\right)  \in\operatorname{Adm} \EE }%
...
} \EE \mid_{Q}\text{-partition}}M_{\operatorname{ev}_{w\mid_{Q}}%
...
\\
&  =\sum_{\left(  P,Q\right)  \in\operatorname{Adm} \EE }\left(
\sum_{\sigma\text{ is a packed } \EE \mid_{P}\text{-partition}%
...
&  =\underbrace{\sum_{\left(  P,Q\right)  \in\operatorname{Adm} \EE %
}\sum_{\sigma\text{ is a packed } \EE \mid_{P}\text{-partition}}%
\sum_{\tau\text{ is a packed } \EE \mid_{Q}\text{-partition}}%
...
P,Q\right)  \in\operatorname{Adm} \EE }\Gamma\left(   \EE %
...
\ \ \ \ \ \ \ \ \ \ \left(  \text{since }f\notin P\setminus\left\{  f\right\}
\right)  \\
& =\left(  P,Q\right)  =\alpha=\operatorname*{id}\left(  \alpha\right)  .
\end{align*}
Hence, $\left(  T\circ T\right)  \left(  \alpha\right)  =\operatorname*{id}%
\left(  \alpha\right)  $ is proven in Case 1.
\par
...
\end{align*}
Hence, $\left(  T\circ T\right)  \left(  \alpha\right)  =\operatorname*{id}%
\left(  \alpha\right)  $ is proven in Case 2.
\par
We have now proven $\left(  T\circ T\right)  \left(  \alpha\right)
=\operatorname*{id}\left(  \alpha\right)  $ in both Cases 1 and 2. Thus,
$\left(  T\circ T\right)  \left(  \alpha\right)  =\operatorname*{id}\left(
\alpha\right)  $ always holds. Qed.}. In other words, $T\circ
T=\operatorname*{id}$. In other words, the map $T$ is an involution.
...
}}\underbrace{{\mathbf{x}}_{\pi\mid_{P},w\mid_{P}}{\mathbf{x}}_{\pi\mid
_{Q},w\mid_{Q}}}_{\substack{=\prod_{e\in E}x_{\pi\left(  e\right)  }^{w\left(
e\right)  }=\mathbf{x}_{\pi,w}\\\text{(since }\mathbf{x}_{\pi,w}=\prod_{e\in
...
Now, we have the equalities $\Phi\circ\Psi=\operatorname*{id}$%
...
}\right)  =\Phi\left(  \pi\right)  =\beta=\operatorname*{id}\left(
\beta\right)  $.
\par
Now, forget that we fixed $\beta$. We thus have proven that $\left(  \Phi
\circ\Psi\right)  \left(  \beta\right)  =\operatorname*{id}\left(
\beta\right)  $ for each $\beta\in\mathfrak{B}$. In other words, $\Phi
\circ\Psi=\operatorname*{id}$, qed.} and $\Psi\circ\Phi=\operatorname*{id}%
...
\alpha\right)  =\alpha^{\circ}\circ\mathfrak{o}=\alpha=\operatorname*{id}%
\left(  \alpha\right)  $.
\par
Now, forget that we fixed $\alpha$. We thus have proven that $\left(
\Psi\circ\Phi\right)  \left(  \alpha\right)  =\operatorname*{id}\left(
\alpha\right)  $ for each $\alpha\in\mathfrak{A}$. In other words, $\Psi
\circ\Phi=\operatorname*{id}$. Qed.}. These two equalities show that the maps
...
\nolimits_{G}\pi$ satisfies $\sign_E g = 1$). Hence, it
...
$\sum_{g\in\operatorname*{Stab}\nolimits_{G}\pi} \sign_E g = 0$. Now,
\[
\dfrac{1}{\left\vert G\right\vert }\underbrace{\sum_{\substack{g\in
G;\\g\pi=\pi}}}_{=\sum_{g\in\operatorname*{Stab}\nolimits_{G}\pi}%
} \sign_E g = \dfrac{1}{\left\vert G\right\vert }%
\underbrace{\sum_{g\in\operatorname{Stab}_{G}\pi} \sign_E g}_{=0}
= 0
=%
\begin{cases}
\dfrac{1}{\left\vert O\right\vert }, & \text{if }O\text{ is }E\text{-coeven}%
...
\end{itemize}
\par
Hence, we can substitute $\Phi\left(\pi\right)$ for $\pi$ in the
sum $\sum_{\overline{\pi}\text{ is an } \EE ^{g}\text{-partition}}
\mathbf{x}_{\pi,w^{g}}$. We thus obtain
\[
\sum_{\overline{\pi}\text{ is an } \EE ^{g}\text{-partition}}
\mathbf{x}_{\pi,w^{g}}
=\sum_{\substack{\pi\text{ is an } \EE \text{-partition;}\\g\pi=\pi
...
\begin{align}
\Gamma\left( \EE , w, G\right)   &  =\sum_{O\text{ is a
}G\text{-orbit on }\operatorname{Par} \EE }\underbrace{{\mathbf{x}%
...
\EE }\sum\limits_{\pi\in O}}_{=\sum_{\pi\in\operatorname{Par}%
...
}}\mathbf{x}_{\pi,w}}_{\substack{=\Gamma\left(   \EE ^{g},w^{g}\right)
...
E\right\vert -\left\vert E^{g}\right\vert }$
(because $E^g$ is the set of all cycles of the permutation of
$E$ that sends every $e\in E$ to $ge$). In other words,
...
\begin{array}
[c]{c}%
\text{here, we have extended the sum to all }G\text{-orbits on }\operatorname{Par} \EE 
\\
...
every $q\in \NN $, let $\operatorname*{Par}\nolimits_{q}\mathbf{E}$
...
_{q} \EE $ is a $G$-subset of the $G$-set $\left\{  1,2,\ldots
...
$\mathbf{k}=\mathbb{Q}$. For any $f\in \QSym $ and any
$q\in \NN $, we define an element $\operatorname*{ps}\nolimits^{1}\left(
...
$\Gamma^{+}\left(   \EE ,w,G\right)  $ of ${\operatorname{QSym}}$
...
,G}\in\mathbb{Q}\left[  X\right]  $ such that every $q\in \NN $ satisfies
...
\end{cases}
\nonumber\\
&  =\sum_{O\text{ is a }G\text{-orbit on }\operatorname{Par}_{q} \EE %
...
But \cite[Proposition 7.7 (iii)]{Reiner} shows that, for any $f\in
 \QSym $ and $m\in \NN $, we have $\operatorname*{ps}%
...
and involutions}, arXiv:1410.5023v4.\newline
...
Drij Grinbrg, \textit{Double posets and the antipode of $\QSym$},
